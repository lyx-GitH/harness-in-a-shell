...
# EXIT: 0
# Wire structural validation into context replacement and verify malformed images
# are rejected without switching execution.
edit_context() {
    local __react_next __react_status

    __react_next="$(mktemp "$ROOT/.react.image.XXXXXX")" || return

    cat > "$__react_next"
    __react_status=$?
    if ((__react_status != 0)); then
        rm -f "$__react_next"
        return "$__react_status"
    fi
...
test_edit_context_rejection() {
    local __react_status

    edit_context <<'BROKEN_IMAGE'
not a complete harness image
BROKEN_IMAGE
    __react_status=$?

    printf 'rejected_status=%d\n' "$__react_status"
    ((__react_status != 0))
}
observe test_edit_context_rejection
reason
# OBS: image validation checks passed
# EXIT: 0
observe test_edit_context_rejection
reason
# OBS: validate_image: missing expected Bash shebang
# OBS: rejected_status=1
# EXIT: 0
# Strengthen finalization: validate both the active image and the extracted
# reusable prefix, and remove staging files on every recoverable failure.
finish() {
    local __react_final __react_status

    validate_image "$SELF" || return

    __react_final="$(mktemp "$ROOT/.react.final.XXXXXX")" || return

    sed -n '1,/^# <TAPE>$/p' "$SELF" > "$__react_final"
    __react_status=$?
    if ((__react_status != 0)); then
        rm -f "$__react_final"
        return "$__react_status"
...
test_finish_staging() {
    local __react_fixture __react_mode

    __react_fixture="$(mktemp "$ROOT/.react-final-test.XXXXXX")" || return
    if ! sed -n '1,/^# <TAPE>$/p' "$SELF" > "$__react_fixture"; then
        rm -f "$__react_fixture"
        return 1
    fi

    if ! validate_image "$__react_fixture"; then
        rm -f "$__react_fixture"
        return 1
    fi

    chmod +x "$__react_fixture" || {
        rm -f "$__react_fixture"
        return 1
    }
    __react_mode="$(stat -c '%A' "$__react_fixture")" || {
        rm -f "$__react_fixture"
        return 1
    }
    rm -f "$__react_fixture"

    printf 'staged prefix valid; mode=%s\n' "$__react_mode"
}
observe test_finish_staging
reason
# OBS: validate_image: missing expected Bash shebang
# OBS: rejected_status=1
# EXIT: 0
...
# OBS: staged prefix valid; mode=-rwx------
# EXIT: 0
# Continue hardening by checking whether Bash startup state can be sanitized
# portably enough for syntax validation and image switching.
audit_clean_bash_environment() {
    env -u BASH_ENV -u ENV -u SHELLOPTS -u BASHOPTS \
        bash --noprofile --norc -c \
        'printf "errexit=%s nounset=%s pipefail=%s bash_env=%q\n" \
            "$(set -o | awk '\''$1=="errexit"{print $2}'\'')" \
            "$(set -o | awk '\''$1=="nounset"{print $2}'\'')" \
            "$(set -o | awk '\''$1=="pipefail"{print $2}'\'')" \
            "${BASH_ENV-}"'
}
observe audit_clean_bash_environment
reason
...
# OBS: errexit=off nounset=off pipefail=off bash_env=''
# EXIT: 0
# Run child Bash processes in a predictable non-interactive environment so
# inherited startup hooks and exported shell options cannot alter validation or
# replacement-image execution.
run_bash_clean() {
    env -u BASH_ENV -u ENV -u SHELLOPTS -u BASHOPTS \
        bash --noprofile --norc "$@"
}
...
test_run_bash_clean() {
    local __react_dir __react_hook __react_marker __react_output

    __react_dir="$(mktemp -d "${TMPDIR:-/tmp}/react-clean-bash.XXXXXX")" || return
    __react_hook="$__react_dir/startup"
    __react_marker="$__react_dir/sourced"

    printf 'printf sourced > %q\n' "$__react_marker" > "$__react_hook" || {
        rm -rf "$__react_dir"
        return 1
    }

    __react_output="$(
        BASH_ENV="$__react_hook" \
            run_bash_clean -c \
    ...
    printf '%s\n' "$__react_output"
    if [[ -e "$__react_marker" ]]; then
        printf 'inherited BASH_ENV was sourced\n'
        rm -rf "$__react_dir"
        return 1
    fi
...
observe test_run_bash_clean
reason
# OBS: errexit=off nounset=off pipefail=off bash_env=''
# EXIT: 0
# Use the sanitized Bash launcher for syntax checks and context-image execution.
validate_image() {
    local __react_image=$1
    local __react_line __react_first=
    local -i __react_line_no=0
    ...
    run_bash_clean -n "$__react_image"
}
...
edit_context() {
    ...
    exec_bash_clean "$__react_next" >> "$__react_next"
    __react_status=$?
    rm -f "$__react_next"
    return "$__react_status"
}

test_clean_image_validation() {
    local __react_dir __react_hook __react_marker __react_fixture

    __react_dir="$(mktemp -d "${TMPDIR:-/tmp}/react-validation.XXXXXX")" || return
    __react_hook="$__react_dir/startup"
    __react_marker="$__react_dir/sourced"
    __react_fixture="$__react_dir/image"

    printf 'printf sourced > %q\n' "$__react_marker" > "$__react_hook" || {
        rm -rf "$__react_dir"
        return 1
    }
    sed -n '1,/^# <TAPE>$/p' "$SELF" > "$__react_fixture" || {
        rm -rf "$__react_dir"
        return 1
    }

    BASH_ENV="$__react_hook" validate_image "$__react_fixture" || {
        rm -rf "$__react_dir"
        return 1
    }
    if [[ -e "$__react_marker" ]]; then
        printf 'validation sourced inherited BASH_ENV\n'
        rm -rf "$__react_dir"
        return 1
    fi

    rm -rf "$__react_dir"
    printf 'clean image validation passed\n'
}
observe test_clean_image_validation
reason
# OBS: errexit=off nounset=off pipefail=off
# EXIT: 0
# OBS: clean image validation passed
# EXIT: 0
